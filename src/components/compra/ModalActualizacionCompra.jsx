...
const ModalActualizacionCompra = ({
  ...
  useEffect(() => {
    if (compra && proveedores.length > 0) {
      setProveedorSeleccionado({ value: compra.id_proveedor, label: compra.nombre_proveedor });
  ...
  const manejarCambioProveedor = (seleccionado) => {
    ...
    setCompraActualizada(prev => ({ ...prev, id_proveedor: seleccionado ? seleccionado.value : '' }));
  };
