...
const ModalRegistroVenta = ({
  ...
  const cargarClientes = (inputValue, callback) => {
    const filtrados = clientes.filter(cliente =>
      `${cliente.nombre_cliente} ${cliente.apellido}`.toLowerCase().includes(inputValue.toLowerCase())
    );
  ...
  const manejarAgregarDetalle = () => {
    ...
    // Verificar stock
    const producto = productos.find(p => p.id_producto === parseInt(nuevoDetalle.id_producto));
    if (producto && parseInt(nuevoDetalle.cantidad) > producto.stock) {
      alert(`Stock insuficiente de ${producto.nombre_}. Unidades disponibles: ${producto.stock}`);
      return;
    }
